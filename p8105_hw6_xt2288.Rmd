...
weather_data_df = weather_df |> 
  ...
  select(.id, r.squared, log_b1_b2, log_b0_b1)
```
...
The distribution of log($\hat{\beta_1} * \hat{\beta_2}$) is left-skewed. NAs are removed.
...
Here is the 95% confidence interval for log($\hat{\beta_0} * \hat{\beta_1}$).
```{r}
quantile(pull(weather_data_df, log_b0_b1), probs = c(0.025, 0.975))
```
